...
# =========================================================================
# CONFIGURACIÓN INICIAL Y ESTADO DE LA SESIÓN
# =========================================================================
...
if opcion == "1. Plan de Cuentas":
    ...
    with col_add:
        ...
        if st.button("Guardar Cuenta"):
            if nueva_cuenta.strip() and nueva_cuenta not in st.session_state.plan_cuentas:
                st.session_state.plan_cuentas.append(nueva_cuenta.strip())
    ...
    with col_list:
        ...
        st.dataframe(pd.DataFrame({"Cuentas Configuradas": st.session_state.plan_cuentas}), use_container_width=True)

# =========================================================================
# MÓDULO 2: CARGA DE ASIENTOS
# =========================================================================
elif opcion == "2. Carga de Asientos":
    st.header("2. Carga de Asientos Contables")
    ...
    with col_debe:
        ...
        cant_debe = st.number_input("Cantidad de Renglones Debe", min_value=1, max_value=5, value=2, key="cant_d")
    ...
    with col_haber:
        ...
        cant_haber = st.number_input("Cantidad de Renglones Haber", min_value=1, max_value=5, value=2, key="cant_h")
    ...
    st.markdown("---")
    if st.button("Registrar Asiento", type="primary"):
        total_debe = sum(r["Monto"] for r in filas_debe)
        total_haber = sum(r["Monto"] for r in filas_haber)

        if total_debe == 0 or total_haber == 0:
            st.error("Error: Debes ingresar montos mayores a cero en al menos un renglón del Debe y del Haber.")
            ...
            # Actualizar Submayores
            if tercero_operacion.strip() not in ["N/A", "Sin especificar", ""]:
                ...
                if tipo_operacion == "Venta":
                    if estado_vencimiento == "Pendiente":
                        ...
                        st.session_state.submayores["Clientes"].append({
                            "Fecha": fecha, "Fecha_Vencimiento": fecha_vencimiento if fecha_vencimiento else fecha,
                            "Cliente": tercero_operacion, "Concepto": f"Venta a Crédito - {concepto}",
                            "Debe (Venta/Cargo)": monto_pendiente, "Haber (Cobro/Pago)": 0.0, "Estado": "Pendiente"
                        })
                        if monto_cobro_efectivo > 0:
                            st.session_state.submayores["Clientes"].append({
                                "Fecha": fecha, "Fecha_Vencimiento": fecha,
                                "Cliente": tercero_operacion, "Concepto": f"Anticipo/Cobro Contado - {concepto}",
                                "Debe (Venta/Cargo)": monto_cobro_efectivo, "Haber (Cobro/Pago)": monto_cobro_efectivo, "Estado": "Cobrado"
                            })
                    else:
                        st.session_state.submayores["Clientes"].append({
                            "Fecha": fecha, "Fecha_Vencimiento": fecha,
                            "Cliente": tercero_operacion, "Concepto": f"Venta Contado - {concepto}",
                            "Debe (Venta/Cargo)": total_debe, "Haber (Cobro/Pago)": total_debe, "Estado": "Cobrado"
                        })

                elif tipo_operacion == "Cobro":
                    st.session_state.submayores["Clientes"].append({
                        "Fecha": fecha, "Fecha_Vencimiento": fecha,
                        "Cliente": tercero_operacion, "Concepto": f"Cobro - {concepto}",
                        "Debe (Venta/Cargo)": 0.0, "Haber (Cobro/Pago)": total_debe, "Estado": "Cobrado"
                    })
                ...
                # Proveedores
                    ...
                    if estado_vencimiento == "Pendiente":
                        ...
                        st.session_state.submayores["Proveedores"].append({
                            "Fecha": fecha, "Fecha_Vencimiento": fecha_vencimiento if fecha_vencimiento else fecha,
                            "Proveedor": tercero_operacion, "Concepto": f"Compra a Crédito - {concepto}",
                            "Debe (Pago)": 0.0, "Haber (Compra/Deuda)": monto_pendiente, "Estado": "Pendiente"
                        })
                        if monto_pago_efectivo > 0:
                            st.session_state.submayores["Proveedores"].append({
                                "Fecha": fecha, "Fecha_Vencimiento": fecha,
                                "Proveedor": tercero_operacion, "Concepto": f"Pago Parcial Contado - {concepto}",
                                "Debe (Pago)": monto_pago_efectivo, "Haber (Compra/Deuda)": monto_pago_efectivo, "Estado": "Pagado"
                            })
                    else:
                        st.session_state.submayores["Proveedores"].append({
                            "Fecha": fecha, "Fecha_Vencimiento": fecha,
                            "Proveedor": tercero_operacion, "Concepto": f"Compra Contado - {concepto}",
                            "Debe (Pago)": total_debe, "Haber (Compra/Deuda)": total_debe, "Estado": "Pagado"
                        })

                elif tipo_operacion == "Pago":
                    st.session_state.submayores["Proveedores"].append({
                        "Fecha": fecha, "Fecha_Vencimiento": fecha,
                        "Proveedor": tercero_operacion, "Concepto": f"Pago - {concepto}",
                        "Debe (Pago)": total_debe, "Haber (Compra/Deuda)": 0.0, "Estado": "Pagado"
                    })
...
# =========================================================================
    ...
    st.header("3. Libro Diario General")
    
    if not st.session_state.asientos:
        st.info("No se han registrado asientos contables aún.")
    else:
        for a in st.session_state.asientos:
            with st.expander(f"Asiento N° {a['Nro']} | Fecha: {a['Fecha']} | Concepto: {a['Concepto']}", expanded=True):
                ...
                with c2:
                    st.write("**HABER**")
                    st.table(df_haber)
                st.write(f"**Total Asiento:** ${a['Total']:,.2f}")

# =========================================================================
# MÓDULO 4: SUBMAYORES Y VENCIMIENTOS
# =========================================================================
elif opcion == "4. Submayores y Vencimientos":
    st.header("4. Gestión de Submayores y Control de Vencimientos")
    ...
    with sub_tab1:
        ...
        if st.session_state.submayores["Clientes"]:
            st.dataframe(pd.DataFrame(st.session_state.submayores["Clientes"]), use_container_width=True)
        else:
            st.info("No hay movimientos cargados en clientes.")
            
    with sub_tab2:
        st.subheader("Submayor de Proveedores")
        if st.session_state.submayores["Proveedores"]:
            st.dataframe(pd.DataFrame(st.session_state.submayores["Proveedores"]), use_container_width=True)
        else:
            st.info("No hay movimientos cargados en proveedores.")

# =========================================================================
# MÓDULO 5: FLUJO DE CAJA PROYECTADO
# =========================================================================
elif opcion == "5. Flujo de Caja Proyectado":
    st.header("5. Proyección de Flujo de Caja por Vencimientos")
    ...
    if not cli_pendientes and not prov_pendientes:
        st.info("No existen operaciones pendientes para proyectar.")
    else:
        flujo = []
        ...
        df_flujo = pd.DataFrame(flujo).sort_values(by="Fecha_Vencimiento")
